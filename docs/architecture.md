...
- הסריקה משתמשת ב־`AtomicBoolean` כדי לוודא שרק תוצאה **אחת** מטופלת גם אם כמה פריימים נסרקים
  כמעט בו־זמנית.
- **דיוק הסריקה** (חבילת `scan/`, לוגיקה טהורה שנבדקת ב־JVM). כל פריים עובר שלושה שלבים לפני
  שערך מתקבל:
    1. `BarcodeValidator` — פוסל ערכים שלא יכולים להיות קריאה שלמה: ספרת ביקורת GS1 ל־EAN-13/EAN-8/UPC-A,
       הרחבה ל־UPC-A ובדיקה ל־UPC-E, ומינימום 4 תווים ל־Code 128/39 (שאין להם ספרת ביקורת חובה).
    2. `AimSelector` — כשכמה קודים בפריים (תווית מדף נושאת **ארבעה** Data Matrix, ולידה ברקודי
    ...
    3. `ScanConsensus` — קוד 1D מתקבל רק אחרי **שני פריימים תואמים** (כ־70ms). ספרת ביקורת תופסת
...
- רזולוציית הניתוח היא 1920×1080 (במקום 1280×720) כדי שהפסים הדקים של EAN יישארו קריאים גם
  ממרחק; לחיצה על התצוגה ממקדת ומודדת חשיפה בנקודה; ו־ML Kit רשאי להגדיל זום אוטומטית (עד ×4)
